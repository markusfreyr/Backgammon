...
try:
    actorData = torch.load("ActorModel_self3")
    actorModel.load_state_dict(actorData)
    actorModel.eval()

    criticData = torch.load("CriticModel_self3")
...
def update(board, player, reward=0):
    alpha = 0.001 # step size for tabular learning
...
def save():
    torch.save(actorModel.state_dict(), 'ActorModel_self2')
    torch.save(criticModel.state_dict(), 'CriticModel_self2')
